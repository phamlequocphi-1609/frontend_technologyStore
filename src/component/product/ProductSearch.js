...
function ProductSearch() {
  ...
  const keySearch = location.state ? location.state.itemSearch : "";
  const dataSearch = location.state ? location.state.suggesData : [];
  ...
  function renderSearchData() {
    if (dataSearch.length > 0) {
      const sortData = sortProduct([...dataSearch]);
          ...
          <div className="col-span-5">
            <div className="grid grid-cols-12">
              <p className="col-span-6 text-[18px] p-2 text-center">
                View all {dataSearch.length} products
              </p>
